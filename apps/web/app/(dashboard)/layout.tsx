...
import { BarChart3, Bell, LayoutDashboard, LogOut, Menu, Settings, Briefcase, X } from 'lucide-react'
      ...
      <div className="px-3 py-4 border-t border-border">
        <div className="flex items-center gap-3 px-3 py-2.5 mb-1">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-muted-foreground truncate capitalize">{plan.toLowerCase()}</p>
          </div>
        </div>
        <button
          onClick={onLogout}
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (user !== null && user.onboardingCompleted === false) {
